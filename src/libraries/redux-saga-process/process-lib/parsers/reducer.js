...
export default function parseSagaProcessReducer(proc, SharedSchema, Compiled) {
  const { schema } = proc;
  if (!schema.config || !schema.config.reduces) {
    return;
  }
  ...
  */
  ...
  if (Array.isArray(schema.config.reduces)) {
    ...
    if (!_.isPlainObject(schema.reducer)) {
      throw new Error(`${
        schema.processID
      } compiltation failed.  When providing an array for config.reducers, the reducer should be a plain object mapping each provided reducer key to a valid process reducer.`);
    }

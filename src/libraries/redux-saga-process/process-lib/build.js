...
/** @module redux-saga-process/build */
 ...
 * @return {CompiledProcessReducers}           [description]
 *
 * buildAsyncProcesses is the first step.  It will evaluate the processes
 * given configuration and build a compiled Process which includes the
 * necessary reducers, selectors, etc.
 */
export async function buildAsyncProcesses(asyncProcesses, globalConfig, configureHandler) {
  // set the default config and values, if previous tasks were
  // known, we will receive a killPromise that resolves when all
  // our previous tasks have been killed.
  // const killPromise = setContextDefaults(globalConfig);
  setContextDefaults(globalConfig);

  const promises = new PromiseMap();

  promises.merge(asyncProcesses);

  const processes = await promises.resolve();
 ...
 */
function formatProcessCategory(category, processCategory, nextCategory, processClass, configureHandler) {
